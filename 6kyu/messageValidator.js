...
//  you need to split the string by the numbers, and then compare the numbers with the number of characters in the
//  following substring.
...
function isAValidMessage(message){
  let arr = message.match(/[A-Za-z]+|\d+/g);
  let result = [];
  let size = 2
...
let check =result.every(sub =>{
  if(!sub[1]) return false;
return sub[1].length === Number(sub[0]);
});
return check;
}
